...
const DashboardMediaHeader = memo( function DashboardPhotosHeader({onSearchChange,url}:DashboardMediaHeader) {
  ...
  return (
  <div className="flex justify-center gap-6  md:justify-between items-center mb-[2%] mx-[1%] rounded-md p-[1%] rtl:flex-row-reverse ">
    ...
    <div className="flex items-center gap-[10%] pr-[8%]  ">
     ...
     ring-4 ring-[#fafafa42] outline-8 outline-[#fafafa28] outline text-nowrap gap-2 "  to={`add${url}`} text={`add ${url}`}  direction={"left"}>
        <IoAddCircleOutline className=' c8  md:c3'/>
      </LinkButton>
    </div>
  </div>
  )
